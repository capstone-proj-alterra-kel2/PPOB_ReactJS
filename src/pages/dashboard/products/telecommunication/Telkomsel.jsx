...
const TelkomselPage = () => {
  ...
  const [Telkomsel, setTelkomsel] = useState([]);
  const token = Cookies.get("token");

  useEffect(() => {
    AxiosInstance.get("/admin/products?size=100&sort=name", {
      headers: {
        Authorization: "Bearer " + token,
      },
    }).then((res) => {
      setLoading(false);
      setTelkomsel(res.data.data.items);
    });
  }, [loading]);

  console.log(Telkomsel);
  ...
  return (
    <SidebarPage>
      <div className="px-10 py-3">
        ...
        <div className="flex flex-col cards flex-wrap">
          {Telkomsel.filter((data) => data.provider_id === 10).map((data) => {
            return (
              <div
                // key={item.id}
              ...
              >
                ...
                <div className=" w-60">
                  ...
                  <div className="text-lg font-semibold">{data.stock}</div>
                </div>
                {/* </div> */}
